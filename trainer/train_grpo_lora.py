...
def calc_reward(args, responses: List[List[str]], contexts: List[List[str]], ground_truth: List[List[str]], answers: List[str], question:List[str], model_engine, tokenizer) -> torch.Tensor:
...
def grpo_train_epoch(epoch, loader, model_engine, tokenizer, retriever, reward_model, start_step, wandb, args):
    for global_step, batch in enumerate(tqdm(loader, initial=start_step + 1, desc=f"Epoch {epoch}"), start=start_step + 1):
                        ...
                        if break_condition(response_token) or len(sub_topics) == 0:
                            ...
                            with torch.no_grad():
                                with model_engine.module.disable_adapter():
                                    response = model_engine.generate(**generator_inputs, max_new_tokens=args.max_gen_len, temperature=1.0, top_p=0.9, top_k=40, num_return_sequences=1, pad_token_id=tokenizer.pad_token_id)
                            response = [
                                output_ids[len(input_ids):] for input_ids, output_ids in zip(generator_inputs.input_ids, response)
                            ]
                            answer = tokenizer.batch_decode(response, skip_special_tokens=True)[0]
        ...
        rewards = calc_reward(args, batch_responses, batch_contexts, batch_gts, batch_answers, question, reward_model, tokenizer)
        ...
        num_steps_local = len(step_input_ids_list)
        if dist.is_initialized():
            num_steps_tensor = torch.tensor([num_steps_local], device=args.device, dtype=torch.long)
            dist.all_reduce(num_steps_tensor, op=dist.ReduceOp.MAX)
            max_num_steps = int(num_steps_tensor.item())
        else:
            max_num_steps = num_steps_local
            
        if num_steps_local < max_num_steps:
            pad_count = max_num_steps - num_steps_local
            ...
            for _ in range(pad_count):
                step_input_ids_list.append(dummy_ids)
                step_masks_list.append(dummy_mask)
                traj_index_for_step.append(0)
        ...
        for chunk_start in range(0, num_steps, args.chunk_size):
            chunk_end = min(chunk_start + args.chunk_size, num_steps)
            ...
            with torch.no_grad():
                with model_engine.module.disable_adapter():
                    ref_logps, _ = compute_logps_merged(
                        model_engine.module, input_ids, attention_mask, completion_mask
                    )
        ...
        if (global_step % args.save_interval == 0 or global_step == iters - 1):
            ...
            if dist.get_rank() == 0:
                peft_model: PeftModel = model_engine.module  # DeepSpeedEngine 包了一层
                save_dir = "../out/RAG_lora_adapter"
                os.makedirs(save_dir, exist_ok=True)
                peft_model.save_pretrained(save_dir)
                Logger(f"LoRA adapter saved to {save_dir}")
    torch.cuda.empty_cache()
    gc.collect()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora_rank", type=int, default=16, help="The LoRA rank")
    parser.add_argument("--lora_alpha", type=int, default=32, help="The LoRA alpha")
    ...
    model_lora = get_peft_model(model_raw, lora_config)
    
    model_lora.print_trainable_parameters()
    # initialize the model with deepspeed
    model_engine, optimizer, _, scheduler = deepspeed.initialize(
        model=model_lora,
        model_parameters=model_lora.parameters(),
        config=ds_config
    ) 
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch == start_epoch and start_step > 0:
            ...
            grpo_train_epoch(epoch, loader, model_engine, tokenizer, retriever,reward_model, start_step, wandb if args.use_wandb else None, args)
        else:
            ...
            grpo_train_epoch(epoch, loader, model_engine, tokenizer, retriever,reward_model, 0, wandb if args.use_wandb else None, args)